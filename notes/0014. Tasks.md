---
id: cf6bf44f-08c5-46e9-8ddd-a06e094751b9
---
  ...
  "tasks": [
    {
      ...
      // 描述，会在命令选择时展示，相当于是这个命令的详情描述。
      "detail": "测试任务 - 输出一个 hello world",
      // 任务类型，shell 表示在终端中执行 shell 命令
      "type": "shell",
      // 要执行的命令
      "command": "echo",
      // 传递给命令的参数列表
      "args": ["hello world"],
      // 任务所属的组，build 表示这是一个构建任务
      "group": "build",
      // 任务执行时的显示选项
      "presentation": {
        // 是否将执行的命令回显到输出
        "echo": true,
        // 何时显示输出面板，always 表示总是显示
        "reveal": "always",
        // 执行任务时是否聚焦到输出面板
        "focus": false,
        // 使用共享面板显示输出
...
```json [.vscode/tasks.json]
{
  ...
  "tasks": [
    ...
    },
    {
      ...
      "dependsOn": ["node-1"] // 运行 2 之前，必须运行 1
    },
    {
      "label": "node-3",
      "type": "shell",
      ...
      "dependsOn": ["node-2"] // 运行 3 之前必须运行 2
    }
  ]
}
```
...
```js [src/2.js]
console.log('2.js run')

// 模拟一个报错，如果在运行到 node-2 的时候发生了错误，那么流程将会结束。
throw new Error('2.js error')
```

```js [src/3.js]
console.log('3.js run')
```
...
```json [tsconfig.json]
{
  // TypeScript 编译器选项
  "compilerOptions": {
    // 指定编译目标 JavaScript 版本为 ES2020
    "target": "ES2020",

    // 指定生成的模块代码类型为 commonjs
    "module": "commonjs",
